...
def gather_20newsgroups_data():
    ...
    def collect_data_from(parent_dir, newsgroup_list): # lay van ban tu tap cac the loai
        data = []
...
def generate_vocabulary(data_path):
    ...
    words_idfs.sort(key = lambda item: -1*item[1]) # ~: -idf
...
def get_tf_idf(data_path):
    with open('../datasets/20news-bydate/words_idfs.txt') as f:
        ...
        word_IDs = dict([(word, index) for index, (word, idf) in enumerate(words_idfs)]) # dict{word:index}

        idfs = dict(words_idfs) # dict{index:word}
        ...
        for document in documents:
            ...
            #words_tfidfs_normalized = [ str(index) + ':' + str(tf_idf_value) for index, tf_idf_value in words_tfidfs]
            # why use sum_squares ??
        ...
        with open('../datasets/20news-bydate/tf_idf.txt', 'w') as f:
            f.write('\n'.join([str(label) + '<fff>' + str(doc_id) + '<fff>' + sparse_rep for label, doc_id, sparse_rep in data_tf_idf]))
        

gather_20newsgroups_data() # xu li van ban
generate_vocabulary('../datasets/20news-bydate/20news-full-processed.txt') # tao dictionary
get_tf_idf('../datasets/20news-bydate/20news-full-processed.txt') # tinh tf-idf
